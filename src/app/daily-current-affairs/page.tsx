...
const CATEGORY_SLUG = "current-affairs"
const PAGE_PATH = "/daily-current-affairs"
...
export async function generateMetadata({ searchParams }: PageProps): Promise<Metadata> {
  ...
  const canonical =
    currentPage > 1
      ...
      : generateCanonicalUrl(PAGE_PATH)
  return {
    title,
    description:
      currentPage > 1
        ? `Current affairs articles for government exams - page ${currentPage}. SSC, RRB, Banking and competitive exam updates.`
        : "Daily current affairs updates for SSC CGL, RRB NTPC, IBPS, SBI and other government exams. Stay updated for your exam preparation.",
...
async function getCurrentAffairsBlogsPaginated(page: number) {
  try {
    const skip = (page - 1) * BLOGS_PER_PAGE
    const where = {
      status: "published" as const,
      category: { in: [CATEGORY_SLUG, "Current Affairs"] },
    }
    ...
    console.error("daily-current-affairs getCurrentAffairsBlogsPaginated:", e)
    return {
      blogs: [],
      pagination: {
        currentPage: 1,
...
function getCategoryLabel(category: string) {
  if (category === CATEGORY_SLUG) return "Current Affairs"
...
function getCategoryBadgeClass(category: string) {
  if (category === CATEGORY_SLUG) return "bg-amber-100 text-amber-800 border-amber-200"
...
export default async function DailyCurrentAffairsPage({ searchParams }: PageProps) {
  const params = await searchParams
  const currentPage = Math.max(1, parseInt(params.page || "1", 10))
        ...
        <main className="max-w-5xl mx-auto" role="main">
          ...
          <div className="mb-8">
            ...
            <p className="text-lg text-gray-600">
              Daily current affairs for SSC, RRB, Banking and other government exams. Stay updated for your exam preparation.
            </p>
          </div>

          {blogs.length === 0 ? (
            <div className="text-center py-12">
              <p className="text-gray-500 text-lg">No current affairs articles yet.</p>
              <Link href="/current-affairs-quiz" className="inline-block mt-4">
                <Button className="mt-4">Try Current Affairs Quiz</Button>
              </Link>
            </div>
          ) : (
            <>
              ...
              {pagination.totalPages > 1 && (
                ...
                >
                  {pagination.hasPrevPage ? (
                    <Link
                      href={pagination.currentPage === 2 ? PAGE_PATH : `${PAGE_PATH}?page=${pagination.currentPage - 1}`}
                  ...
                  {Array.from({ length: Math.min(5, pagination.totalPages) }, (_, i) => {
                    ...
                    return (
                      <Link
                        key={pageNum}
                        href={pageNum === 1 ? PAGE_PATH : `${PAGE_PATH}?page=${pageNum}`}
                  ...
                  {pagination.hasNextPage ? (
                    <Link
                      href={`${PAGE_PATH}?page=${pagination.currentPage + 1}`}
                      className="px-3 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 transition-colors"
                    >
                      Next
                    </Link>
