...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const canonicalPath = `/rrb/${examType}/syllabus`
  const title = `${name} Syllabus | pkminfotech`
  const description = `${name} exam syllabus, pattern and section-wise topics. Free syllabus for Railway exam preparation.`
  return {
    title,
    description,
