...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const canonicalSlug = config?.slug ?? (examRecord ? getExamTypeSlug(examRecord.slug, "rrb") : examType)
  const canonicalPath = `/rrb/${canonicalSlug}`
  const title = `${name} | Practice, Mock Test, PYQ | pkminfotech`
  const description = `Free ${name} practice sets, mock tests, daily quiz, PYQ and syllabus. Start your RRB exam preparation with topic-wise practice and full mocks.`
  return {
    title,
    description,
