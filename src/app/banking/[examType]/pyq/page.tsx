...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const canonicalPath = `/banking/${examType}/pyq`
  const title = `${name} PYQ | Previous Year Questions | pkminfotech`
  const description = `Free ${name} previous year question (PYQ) practice. Topic-wise PYQ with timer and negative marking for bank exam preparation.`
  return {
    title,
    description,
...
export default async function BankingExamPyqPage({ params }: PageProps) {
  const { examType } = await params
  const config = getBankingExamTypeBySlug(examType)
  const examRecord = await resolveExamByCategoryAndSlug("banking", examType)
  ...
  if (canonicalTypeSlug !== examType) redirect(`/banking/${canonicalTypeSlug}/pyq`)
  ...
  return (
    <main className="bg-light py-4">
      <div className="container" style={{ maxWidth: 1120 }}>
        ...
        <PyqSeoContent displayName={displayName} base={base} categoryLabel="Banking" categoryHref="/banking" />
      </div>
    </main>
  )
}
