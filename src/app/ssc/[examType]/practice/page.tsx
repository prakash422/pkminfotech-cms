...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: `${name} Practice | Question Sets | PKMinfotech`,
    description: `Free ${name} practice question sets and MCQ.`,
  }
}

export default async function SscExamPracticePage({ params }: PageProps) {
  const { examType } = await params
  const config = getSscExamTypeBySlug(examType)
  const examRecord = await resolveExamByCategoryAndSlug("ssc", examType)
      ...
      <div className="container" style={{ maxWidth: 1120 }}>
        ...
        <section className="card border-0 shadow-sm mb-4">
          <div className="card-body p-4 p-md-5">
            ...
            <p className="text-secondary mb-0">Question sets for {displayName}. Start with any set.</p>
          </div>
        </section>
        ...
        {sets.length === 0 ? (
          <section className="card border shadow-sm">
            <div className="card-body p-4 text-secondary">
              Practice sets for {displayName} will be added here soon. Check back later.
            </div>
          </section>
        ) : (
